...
A derivation of \refEq{ODEs} is presented in \cite{DT}; our \refEq{ODEs} corresponds to their equations 8.114 and 8.115. As stated in \cite[][p.~270]{DT}, the traction, $\bt$, exerted on any spherical surface is given in terms of the displacment scalars $U$, $V$, and $W$ by
%
\begin{equation}
...
\begin{itemize}
\item A key point is to recognize that ``the mode'' $_n\ssT_l$ comprises three parts:
%
\begin{equation}
...
So ``the mode'' or ``the solution'' $_n\ssT_l$ (to \refeq{ODEs}) comprises three parts: an eigenfrequency, $\omnl$, and two eigenfunctions, $W$ and $T$, with the boundary condition $T(a) = T(b) = 0$:
%
\begin{equation}
_n\ssT_l \hspace{10pt} \left\{
\begin{tabular}{ll}
...
%
\begin{enumerate}
\item Write down the two equations represented by \refEq{ODEs} and the equations for the boundary conditions; show explicit $r$ dependence, $l$ dependence, and $n$ dependence (\eg $\omnl$).
...
\item (0.0) Execute the script \verb+spshell_template.m+ and analyze the output. Two figures are generated, one for a set of $W(r)$, the other for a set of $T(r)$ (\refFig{fig:Tr}).
%
\begin{enumerate}
...
\item The script is looping over a set of $(l_i, f_i)$ values. In \refFig{fig:dots}a, plot (by hand) 14 dots that represent your values.
...
%
\begin{enumerate}
\item (0.2) In \refFig{fig:dots}b, plot (by hand) the function evaluations that you performed.

Hint: what is $f_{\rm min}$?

\item (0.1) List the predicted eigenperiods in \refTab{tab:mode_pred}.

\item (0.2) Among the toroidal modes in \refTab{tab:mode_pred}, the gravest mode is $_0\ssT_2$ ($n=0$, $l=2$).
%
\begin{itemize}
\item List the eigenfrequency $_0f_2$ in mHz and eigenperiod $_0T_2$ in seconds and minutes. (Note \footnote{Notation might be confusing: $T$ is period, $T(r)$ is the stress eigenfunction (for toroidal modes), and $_n\ssT_l$ is the label for a toroidal mode. Usually it should be clear from the context.})

\item Compare with the observed value in \refTab{tab:mode_obs}. What is the percent difference, $100\,\ln(T/T_{\rm obs})$, where $T$ is your computed eigenperiod for $_0\ssT_2$?
\end{itemize}
...
\item (0.3) On your dispersion diagram, plot the observations in \refTab{tab:mode_obs} with a different symbol (or color) from the predictions.
...
\item (0.4) Compute $F(\bem)$ for all available observations (\refTab{tab:mode_obs}), and list the value in mHz in \refTab{tab:homo}. These observations will be loaded within \verb+spshell_template.m+ if you comment out the last \verb+break+ statement.
...
Show all predictions and all observations in your plot, not just the ones used in calculating the misfit.

\item (0.3) Repeat, but now use only the observed eigenfrequency for $_0\ssT_2$. (This means that all but one of the observations in your plot will not have been used in determining the new~$\mu$.)
Show all predictions and all observations in your plot, not just the ones used in calculating the misfit.

What is the percent difference from the observed value, $100\,\ln(f/f_{\rm obs})$, for $_0\ssT_2$?
...
\item (1.0) {\bf Linear earth.} Adapt \verb+earthfun.m+ (\refSec{sec:earthfun}) to create a function that inputs a radial value $r$ (or a vector of $r$ values) and outputs $\rho(r)$ and $\mu(r)$ described by a linear model with the ``endpoint'' values
%
\begin{eqnarray*}
...
\item How has the cubic model affected the depth sensitivity, $W(r)$, of $_0\ssT_{40}$?
...
The phase speed, $c$, and the group speed, $U$, have units of rad/s. Multiplication by $a$ (earth radius) will give the group speed in km/s ($U' = Ua$). List both $U$ and $U'$ in \refTab{tab:eigfun}.
...
The second misfit value is the same model, but computing $F(\bem)$ for all $N_{\rm total}=20$ observations in \refTab{tab:mode_obs}.
