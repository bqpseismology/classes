...
\newcommand{\omnl}{\mbox{${}_{n\hspace{-0.2 mm}}\omega_l$}}  % eigfreq
\newcommand{\fnl}{\mbox{${}_{n\hspace{-0.2 mm}}f_l$}}        % eigfreq
...
\item The necessary starting Matlab scripts and data are available from \verb+seis2016+ directory on the linux network; type \verb+svn update+ to obtain the latest version of files. The relevant files are:
%
\begin{itemize}
...
So ``the mode'' or ``the solution'' \tnl{n}{l} (to \refeq{ODEs}) comprises three parts: an eigenfrequency, $\omnl$, and two eigenfunctions, $W$ and $T$, with the boundary condition $T(a) = T(b) = 0$:
%
\begin{equation}
\tnl{n}{l} \hspace{10pt} \left\{
\begin{tabular}{ll}
...
\item (0.2) Among the toroidal modes in \refTab{tab:mode_pred}, the gravest mode is \tnl{0}{2} ($n=0$, $l=2$).
%
\begin{itemize}
...
\item (0.4) Compute $F(\bem)$ for all available observations (\refTab{tab:modes_obs}), and list the value in mHz in \refTab{tab:homo}. These observations will be loaded within \verb+spshell_template.m+ if you comment out the last \verb+break+ statement.
...
\pagebreak
\item (1.0) Check that each of the equations in \refeq{ODEs} is satisfied for \tnl{0}{40}: $_0W_{40}(r)$, $_0T_{40}(r)$, $_0\omega_{40}$.
...
\caption[]
{{
Wave parameters (\refTab{tab:waveparm}) for \tnl{0}{40} ($n=0$, $l=40$) addressed in Problems 1-\ref{prob:0T40_homo} and 3-\ref{prob:0T40_cubic}. List your values with three significant figures.
\label{tab:eigfun}
}}
\begin{spacing}{1.5}
\hspace{-1.0cm}
