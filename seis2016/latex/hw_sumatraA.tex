...
\item (0.2) \refEq{fftder1} holds for the first derivative of $\fft(t)$, which we denote as either $\dot{\fft}(t)$ or $\fft^{(1)}(t)$. The expression can be generalized for the $n$th derivative, which is (using two different notations)
%
\begin{eqnarray}
...
\item (0.4) Using the same frequencies that you used to define $C(\omega)$ in hte previous problem, evaluate the instrument response to acceleration, $I_a(\omega)$. Hint: see lab on instrument response.
...
%\pagebreak
\item (1.5) {\bf P wave}. Rename \verb+CAN_P.m+, then adapt the script to answer the following questions.
%
\begin{enumerate}
...
\item (1.0) {\bf High-frequency bandpass}. Rename \verb+CAN_bp.m+, then adapt the script to answer the following questions.
%
\begin{enumerate}
