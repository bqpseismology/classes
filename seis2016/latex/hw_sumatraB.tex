...
\item Make sure you have done the examples in \verb+doc_startupB.pdf+ and are familiar with what \verb+getwaveform.m+ and \verb+plotw_rs.m+ are doing. Be sure you understand what a bandpass filter is.

%---------

\item Make sure you have completed the lab on instrument response (\verb+lab_response.pdf+).
...
\item (0.0) Browse \verb+all_sumatra_modes.pdf+ to pick a subset of at least 20 stations for this analysis. The stations should be selected based on high signal-to-noise within the frequency range 0.2 to 1.0~mHz, and, notably, the \snl{0}{2} peak. Be sure to include Canberra and the station from Figure~1 of \citet{Park2005}.
{\bf See ``Tips on picking stations'' in the instructions above.}

No need to list the stations here; you'll be plotting them later.

%---------

\item (0.5) Set \verb+iload = 0+ in \verb+sumatra_modes_template.m+, then modify \verb+ipick+ (or use \verb+wkeep.m+) and plot your selected stations.
...
\item (0.5) \input{hw_sumatraB_CAN}
