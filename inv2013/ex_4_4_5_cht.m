...
addpath('/usr/local/matlab_toolboxes/aster/cd_5.2/Examples/chap4/ex_4_4_5/');
...
% confidence interval
sigmaM = sigma*sqrt(diag(inv(G'*G)));
dm = 1.96*sigmaM;

%plot the least-squares solution and confidence intervals
figure(2); hold on;
plotconst_mod(mls*1000-dm*1000,0,maxdepth,{'r--'});
plotconst_mod(mls*1000+dm*1000,0,maxdepth,{'r--'});
plotconst_mod(mls*1000,0,maxdepth,{'k','linewidth',2});
...
hold off
text(15,0.02,'1^{st} order','FontSize',18);
text(28,0.00001,'2^{nd} order','FontSize',18);
ylabel('Filter Factors');
xlabel('index, i');
fontsize(fsize);
%print -deps2 c4tikhfilt.eps
disp('Displaying the filter factors for first and second-order regularization (fig. 7)')

% display the recovered model misfits
dinterp = dd/2:dd:1000-dd/2;
strueresamp = slowfunctionsmooth(dinterp)';

disp(['1st-order model 2-norm misfit is ' num2str(norm(m1-strueresamp)) ' s/km']);
disp(['2nd-order model 2-norm misfit is ' num2str(norm(m2-strueresamp)) ' s/km']);
